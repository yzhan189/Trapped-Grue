...
(def the-map  ;  15 rooms   14 unconditional objs + 3 conditional objs
...
You get the feeling it was just created for a game or something."
   ...
   :grue-pen {:desc "It is very dark.  You are about to be eaten by a grue. Are you going to fight? "
              ...
              :hint "Do you know that grue loves raw eggs?"
   ...
   :basement { :title "in the basement"
               ...
               :contents #{:spirit3 }}
   :DNE {:title "in The Room Does Not Exist"
         :hint "Stay."
...
(defn status [player]  ;status associate
...
(defn help [player]
  (do (println "Try: look -- see where you are")
      ...
      (println "     pickup, use -- do something with objects")
        ...
        (do (println "The more grue loves you, the more tips you will get.")
            (if (> (player :grue-love) 1)
              (do
                ...
                (println "     cook -- your energy increments")
                (println "     mix -- use beaker to mix pieces of spirits")
  ...
  (let [location (player :location)]
    (if  (= location :kitchen )
      (do (if (contains? (player :inventory) :raw-egg)
            (do (println "Since there is no other food in kitchen, you just have some boiled eggs.")
...
(defn respond [player command] ; 19 commands
  (match command
         ...
         [:pickup] (pickup player) ;recieve item
         [:mix] (mix player)
         [:ask] (ask player)
         [:cook] (cook player)
         [:feed] (feed player)
